...
//    private String total_profit_rate; // 标的总年化利率:原始+赠送利率
    ...
    private RegularDetailsInfo mInfo;
    ...
    public void initTitle(WFYTitle mTitle) {
        switch (prodId) {
            case Constants.PRODUCT_TYPE_REGULAR_PROJECT:
                mTitle.setTitleText("定期项目");
                break;
//            case RegularBase.REGULAR_04:
//                mTitle.setTitleText("定期项目转让");
//                break;
            case Constants.PRODUCT_TYPE_REGULART_PLAN:
...
//                return "定期计划转让详情";
    ...
    public void obtainData() {
        ...
        HttpRequest.getRegularDetail(mContext, subjectId, new ICallback<MqResult<RegularDetailsInfo>>() {

            @Override
            public void onSucceed(MqResult<RegularDetailsInfo> result) {
                synchronized (mLock) {
                    inProcess = false;
                }
...
//        }
    ...
    private void updateRegularCom() {
        ...
        if (mInfo.getBidType().equals(Constants.BID_TYPE_SBB)) {
            iv_tag.setImageResource(R.drawable.double_rate_card_normal);
        } else if (mInfo.getBidType().equals(Constants.BID_TYPE_SBSYK)) {
...
//        }
        ...
        tv_profit_rate.setText(mInfo.getProductRate());
        if (Constants.BID_TYPE_SBB.equals(mInfo.getBidType())) {
            tv_profit_rate.setText(mInfo.getYearRate());
            tv_profit_rate_unit.setText("%");
        } else if ((new BigDecimal(mInfo.getAddInterestRate()).compareTo(BigDecimal.ZERO)) == 1) {
            tv_profit_rate_unit.setText("+ " + mInfo.getAddInterestRate() + "%");
        }else {
            tv_profit_rate_unit.setText("%");
        }
    ...
    private void updateRegularProjectFeature() {
        ...
        ArrayList<RegularProjectFeature> mList = mInfo.getRegularFeatures();
    ...
    private View.OnLayoutChangeListener onLayoutChangeListener = new View.OnLayoutChangeListener() {
        ...
        public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
            if (Constants.getCurrentStatus(mInfo.getStatus()) != Constants.STATUS_YKB) {
                return;
            }
